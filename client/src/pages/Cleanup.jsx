...
const Cleanup = () => {
  ...
  const handleCleanup = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/api/enhance/cleanup`, formData, {
        headers: {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50/30 pt-20'>
      <motion.div className='container mx-auto px-4 sm:px-10 md:px-14 lg:px-28 py-16 min-h-screen flex flex-col items-center justify-center' initial="hidden" animate="visible" variants={containerVariants}>
        <motion.h1 className='text-3xl md:text-4xl font-bold text-gray-800 mb-4 text-center' variants={itemVariants}>AI Cleanup</motion.h1>
        <motion.p className='text-gray-600 max-w-2xl text-center mb-12 text-lg' variants={itemVariants}>Remove unwanted objects and imperfections with AI-powered cleanup.</motion.p>
        ...
        <motion.div className='w-full max-w-4xl flex flex-col items-center' variants={itemVariants}>
          ...
          <motion.div className='grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl' variants={itemVariants}>
            ...
            <div className='bg-white p-6 rounded-xl shadow-lg'>
              <h3 className='text-lg font-semibold mb-4 text-center text-gray-800'>Cleaned Up</h3>
              <div className='aspect-square bg-gray-100 rounded-lg flex items-center justify-center'>
                {loading ? (<div className='flex flex-col items-center'><div className='w-12 h-12 border-4 border-primary-500 border-t-transparent rounded-full animate-spin mb-4'></div><p className='text-gray-600'>Cleaning up...</p></div>) : processedImage ? (<img src={processedImage} alt='Processed' className='max-w-full max-h-full object-contain rounded-lg' />) : (<p className='text-gray-500'>Processed image will appear here</p>)}
              </div>
            </div>
          </motion.div>

          <motion.button onClick={handleCleanup} disabled={!selectedFile || loading} className='mt-8 bg-gradient-to-r from-primary-600 to-secondary-500 text-white px-8 py-3 rounded-full font-medium shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2' variants={itemVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.98 }}>
            {loading ? (<><div className='w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin'></div>Cleaning...</>) : (<>Cleanup</>)}
          </motion.button>
        </motion.div>
      </motion.div>
    </div>
